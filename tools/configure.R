...
#   Name of the GitHub repo (e.g. yutannihilation/wgpugd)
#
# Config/<package name>/crate_name (optional):
#   Name of the crate (e.g. wgpugd). If the crate name is the same as the
...
#   Example:
#       list(
#           `aarch64-apple-darwin-libwgpugd.a`       = "4a34f99cec66610746b20d456b1e11b346596c22ea1935c1bcb5ef1ab725f0e8",
#           `i686-pc-windows-gnu-libwgpugd.a`        = "ceda54184fb3bf9e4cbba86848cb2091ff5b77870357f94319f9215fadfa5b25",
#           `ucrt-x86_64-pc-windows-gnu-libwgpugd.a` = "26a05f6ee8c2f625027ffc77c97fc8ac9746a182f5bc53d64235999a02c0b0dc",
#           `x86_64-apple-darwin-libwgpugd.a`        = "be65f074cb7ae50e5784e7650f48579fff35f30ff663d1c01eabdc9f35c1f87c",
#           `x86_64-pc-windows-gnu-libwgpugd.a`      = "26a05f6ee8c2f625027ffc77c97fc8ac9746a182f5bc53d64235999a02c0b0dc"
#       )
...
check_cargo <- function() {
  ...
  if (!isTRUE(res_version$success)) {
    stop(errorCondition("cargo command is not available", class = c("wgpugd_error_cargo_check", "error")))
  }
  ...
  if (identical(SYSINFO_OS, "windows")) {
    ...
    if (!isTRUE(res_version$success)) {
      stop(errorCondition(
        ...
        class = c("wgpugd_error_cargo_check", "error")
      ))
    }
  }
  ...
  if (isTRUE(!is.na(msrv))) {
    ...
    if (length(m) != 2) {
      stop(errorCondition("cargo version returned unexpected result", class = c("wgpugd_error_cargo_check", "error")))
    }
    ...
    if (package_version(m[2]) < package_version(msrv)) {
      ...
      stop(errorCondition(msg, class = c("wgpugd_error_cargo_check", "error")))
    }
  }
...
download_precompiled <- function() {
  ...
  if (isTRUE(is.na(github_repo) || is.na(github_tag) || is.na(crate_name))) {
    ...
    stop(errorCondition(msg, class = c("wgpugd_error_download_precompiled", "error")))
  }
...
cargo_check_result <- tryCatch(
  ...
  wgpugd_error_cargo_check = function(e) e$message
)
...
download_precompiled_result <- tryCatch(
  download_precompiled(),
  # Defer errors if it's raised by functions here
  wgpugd_error_download_precompiled = function(e) e$message
)
